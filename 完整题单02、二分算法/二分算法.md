---
title: "【完整题单02、二分算法】【✅✅✅✅】"
original_url: https://blog.csdn.net/weixin_51429773/article/details/130000330
published: 2026-06-14
views: 粉丝可见
csdn_article_id: 130000330
---

# 【完整题单02、二分算法】【✅✅✅✅】

> 来源：[【完整题单02、二分算法】【✅✅✅✅】](https://blog.csdn.net/weixin_51429773/article/details/130000330) · 发布时间：2026-06-14 · 阅读量：粉丝可见
...
public:
    bool searchMatrix(vector<vector<int>>& matrix, int target) {
        ...
        int all = m * n;
        int l = 0, r = all - 1;
        int mid = 0, m_p = 0, n_p = 0;
        while(l <= r){
            mid = (l + r) / 2;
            m_p = mid / n;
            n_p = mid % n;
            if(target < matrix[m_p][n_p]){
                r = mid - 1;
            }else if(target == matrix[m_p][n_p]){
                break;
            }else if(target > matrix[m_p][n_p]){
                l = mid + 1;
            }
        }
        if(matrix[m_p][n_p] == target)return true;
        return false;
    }
};
```
...
public:
    vector<int> searchRange(vector<int>& nums, int target) {
        ...
        if(target == nums[mid]){
            l_f = mid;
            while(l_f >= 0 && (nums[l_f] == target)){
                l_f--;
            }
            r_f = mid;
            while(r_f < n && (nums[r_f] == target)){
                r_f++;
            }
            res[0] = l_f + 1;
            res[1] = r_f - 1;
